$ ->
  ...
  class Archives
    constructor: (params)->
      ...
      @_$counter = @_$target.find "#js-archivesCounter"
      ...
      @_setItem()
      @_$carousel.carousel()
      @_$counter.html 1 + " / " + @_archives.length
    ...
    _completeSlide: (e)=>
      ...
      index = Number($item.attr("index")) + 1
      @_$counter.html(index + " / " + @_archives.length)
      @_$data.stop().velocity {
        opacity: [1, 0]
      }, {
        duration: 500
        easing: "linear"
  ...
  if paramStr
    ...
    for param in paramArr
      if param.split("=")[0] is "reload"
        if param.split("=")[1] is "1"
