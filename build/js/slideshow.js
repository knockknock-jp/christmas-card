...
$(function() {
  var $body, $document, $html, $window, Archives, EVENT_OPEN_MODAL, ID_MODAL_GET_ARCHIVES, Modal, Values, _archives, _modal, archives, j, len, modal, onResize, param, paramArr, paramStr, values;
  $window = $(window);
  $html = $("html");
  $document = $(document);
  $body = $("body");
  ...
  Archives = (function() {
    function Archives(params) {
      ...
      this._$counter = this._$target.find("#js-archivesCounter");
      this._archives = values.getArchives();
      this._arr = [];
      this._$items.empty();
      this._setItem();
      this._$carousel.carousel();
      this._$counter.html(1 + " / " + this._archives.length);
      ref = this._archives;
    ...
    Archives.prototype._startSlide = function(e) {
      var $item, data, flg, index, j, len, ref;
      $item = $(e.relatedTarget);
      this._$data.velocity({
        opacity: [0, 1]
      }, {
        duration: 200,
        easing: "linear"
      });
      index = Number($item.attr("index")) + 1;
      flg = true;
      ref = this._arr;
      for (j = 0, len = ref.length; j < len; j++) {
        data = ref[j];
      ...
      if (flg) {
        if ($("#js-checkbox").is(":checked")) {
          location.href = "./slideshow.html?reload=1";
        }
      }
      return this._arr[index - 1] = true;
    };
    ...
    Archives.prototype._completeSlide = function(e) {
      ...
      index = Number($item.attr("index")) + 1;
      this._$counter.html(index + " / " + this._archives.length);
      return this._$data.stop().velocity({
        opacity: [1, 0]
      }, {
        duration: 500,
        easing: "linear"
  ...
  archives.Item = (function() {
    function Item(params) {
      ...
      this._$target.attr("index", this._index);
    }
  ...
  _archives = null;
  paramStr = String(location.href).split("?")[1];
  if (paramStr) {
    paramArr = paramStr.split("&");
    for (j = 0, len = paramArr.length; j < len; j++) {
      param = paramArr[j];
      if (param.split("=")[0] === "reload") {
        if (param.split("=")[1] === "1") {
